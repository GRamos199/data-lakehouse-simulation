...
from config.config import TIMEZONE, ANALYTICS_DATA_PATH
from src.ingestion.api_ingestion import APIIngestionManager
from src.ingestion.csv_ingestion import CSVIngestionManager
from src.transformations.raw_to_clean import RawToCleanTransformer
from src.transformations.clean_to_analytics import CleanToAnalyticsTransformer
...
class DataLakehousePipeline:
    ...
    def run_ingestion_stage(self) -> bool:
        ...
        try:
            ...
            sample_csv = Path(__file__).parent.parent / "data" / "raw" / "csv" / "sample_historical.csv"
    ...
    def run_analytics_stage(self) -> bool:
        ...
        try:
            ...
            if transform_result.get("status") == "success":
                ...
                logger.info(f"  - Exported files: {len(transform_result.get('exported_files', []))}")
                ...
                if summary:
                    logger.info("\n[Analytics Summary]")
                    logger.info(f"  Cities loaded: {len(summary.get('cities_loaded', []))}")
                    logger.info(f"  Total records: {summary.get('weather_analytics_rows', 0)}")
                ...
                logger.warning("⊘ Analytics tables not created (likely no clean data available)")
    ...
    def print_execution_summary(self):
        ...
        logger.info("\nNext steps:")
        logger.info("  1. Review the data in each layer: data/raw, data/clean, data/analytics")
        logger.info("  2. Query the DuckDB database: data/analytics/lakehouse.duckdb")
        logger.info("  3. Check the analytics report: data/analytics/analytics_report.json")
        logger.info("=" * 80)
    ...
    def run(self) -> bool:
        ...
        return all(success for _, success in stages_successful[:2])  # At least ingestion and transformation
